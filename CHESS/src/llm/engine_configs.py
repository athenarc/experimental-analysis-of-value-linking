...
ENGINE_CONFIGS: Dict[str, Dict[str, Any]] = {
    ...
    "llama_3_3_70b_vllm": {
        "constructor": ChatOpenAI,
        "params": {
            "model": "kosbu/Llama-3.3-70B-Instruct-AWQ", # The name of the model you pulled with `ollama pull`
            ...
            "max_tokens": 8192,         # Adjust as needed for Qwen3's capability and prompt length
            "temperature": 0.0
        },
        # "preprocess": lambda x: x.to_messages() # Usually not needed for ChatOllama if prompts are already ChatMessages
    },
    "qwen3_32b_vllm": {
        "constructor": ChatOpenAI, # We use ChatOpenAI to talk to vLLM's OpenAI-compatible API
        "params": {
            "model": "Qwen/Qwen3-32B",  # This MUST match the model name vLLM is serving
            "openai_api_key": "EMPTY",  # vLLM doesn't require an API key by default
            "openai_api_base": "http://localhost:5001/v1", # Default vLLM API endpoint. Adjust if your vLLM server is on a different host/port.
            "max_tokens": 4096,         # Adjust as needed for Qwen3's capability and prompt length
            "temperature": 0.0
        }
    },
    "qwen3_coder_vllm": {
    ...
    "Qwen/Qwen2.5-Coder-32B-Instruct-GPTQ-Int8": {
        "constructor": ChatOpenAI, # We use ChatOpenAI to talk to vLLM's OpenAI-compatible API
        "params": {
            "model": "Qwen/Qwen2.5-Coder-32B-Instruct-GPTQ-Int8",  # This MUST match the model name vLLM is serving
            "openai_api_key": "EMPTY",  # vLLM doesn't require an API key by default
            "openai_api_base": "http://localhost:5001/v1", # Default vLLM API endpoint. Adjust if your vLLM server is on a different host/port.
            "max_tokens": 2048,         # Adjust as needed for Qwen3's capability and prompt length
            "temperature": 0.7
