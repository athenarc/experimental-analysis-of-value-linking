...
# --- Start of changes ---
# Module-level store for the shared embedding function instance
_shared_embedding_function_instance = None
_shared_embedding_function_lock = threading.Lock()

def get_shared_embedding_function():
    """
    Returns a shared instance of HuggingFaceEmbeddings.
    Initializes it on the first call within a process.
    """
    global _shared_embedding_function_instance
    if _shared_embedding_function_instance is None:
        with _shared_embedding_function_lock:
            if _shared_embedding_function_instance is None:
                print("INFO: Initializing shared HuggingFaceEmbeddings for RetrieveEntity...")
                torch.cuda.empty_cache() # Clear cache before first load
                _shared_embedding_function_instance = HuggingFaceEmbeddings(
                    model_name="BAAI/bge-m3",
                    model_kwargs={'device': 'cpu'}, # Force CUDA
                    encode_kwargs={'normalize_embeddings': True}
                )
                print("INFO: Shared HuggingFaceEmbeddings for RetrieveEntity initialized and loaded to CUDA.")
    return _shared_embedding_function_instance
# --- End of changes ---
...
class RetrieveEntity(Tool):
    ...
    def __init__(self):
        super().__init__()
        # torch.cuda.empty_cache() # Removed from here, moved to the singleton getter
        self.embedding_function = get_shared_embedding_function() # Use the shared instance
    ...
    def _get_similar_entities(self, keywords: List[str]) -> Dict[str, Dict[str, List[str]]]:
        ...
        for table_name, column_values in selected_values.items():
            for column_name, values in column_values.items():
                if not values: continue # Add check for empty list
                ...
                if not values: continue # Add check for empty list after filtering
    ...
    def _get_similar_entities_via_embedding(self, similar_entities_via_edit_distance: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        if not to_embed_strings: # Add check for empty list
            return []
