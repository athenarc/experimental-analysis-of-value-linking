...
RETRIEVAL_DEPTH = 1000 # Retrieve a fixed large number for full evaluation

# Model Configuration
LLM_MODEL_PATH = "Qwen/Qwen3-4B-Instruct-2507"
...
def load_and_group_benchmark_data(
    ...
    # Use the global DB_CONFIG name as default since JSON lacks it
    ...
    for task in all_tasks:
        ...
        # 3. FIX: Extract the actual perturbation type value
        category = "uncategorized"
        if isinstance(changes_info, dict):
            # Try to get the explicit perturbation type value
            if "perturbation_type" in changes_info:
                category = changes_info["perturbation_type"]
            else:
                # Fallback to previous logic if perturbation_type key isn't there
                category_keys = [k for k in changes_info if k not in ["template_value", "new_clean_value", "new_perturbed_value"]]
                if category_keys:
...
def get_system_configs():
    ...
    #    query_processor=ChessQueryProcessor(model_name_or_path=LLM_MODEL_PATH, cache_folder="./cache/keywords_chess", tensor_parallel_size=2, gpu_memory_utilization=0.50),
    ...
    #    query_processor=OpenSearchQueryProcessor(model_name_or_path=LLM_MODEL_PATH, cache_folder="./cache/keywords_open_search", tensor_parallel_size=1, gpu_memory_utilization=0.60),
    ...
    # Helper to create the Postgres Loader
    def create_openaire_loader(db_id_unused):
        # We ignore the db_id passed from the loop because we are fixed to the Postgres config
        return OpenAireLoader(db_config=DB_CONFIG, schema=SCHEMA_NAME, max_values=-1)

    configs = {
        #"CHESS": {
        #    "searcher": chess_searcher,
        #    "get_db_specifics": lambda db_id: {
        #        "loader": create_openaire_loader(db_id),
...
def main():
    ...
    for system_name, system_config in system_configs.items():
        ...
        for db_id, (queries, gold_standard, categories, sources) in benchmark_data.items():
            ...
            # Pass the OpenAireLoader to the search method
        ...
        # Save predicted values for this system
        output_json_path = PREDICTED_VALUES_FILE.replace(".json", f"_{system_name}.json")
