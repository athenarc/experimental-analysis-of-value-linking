...
def retrieve_relevant_hits(searcher, queries):
    ...
    for query, q_id in zip(queries, q_ids):
        current_hits_objects = search_results.get(q_id, [])
        
        raw_json_strings = []
        for hit_obj in current_hits_objects:
            if hit_obj.lucene_document:
                raw_content_string = hit_obj.lucene_document.get("raw")
                if raw_content_string:
                    raw_json_strings.append(raw_content_string)
                    
        unique_raw_json_strings = list(dict.fromkeys(raw_json_strings))
        
        parsed_hits = []
        for json_str in unique_raw_json_strings:
            try:
                parsed_hits.append(json.loads(json_str))
            except json.JSONDecodeError as e:
                print(f"Error decoding JSON for query '{query}', q_id '{q_id}'. String: '{json_str}'. Error: {e}")
