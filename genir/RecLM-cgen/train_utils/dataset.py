...
class ValueLinkingDataset(Dataset):
    def __init__(self, args, task_template, task_num, data, tokenizer, mode):
        if not hasattr(args, 'process_index'):
            args.process_index = 0
        self.args = args
        self.task_template = task_template
        ...
        for v in self.canonical_values_meta:
            self.canonical_values_meta[v][self.args.item_index] = v
        
        self.title2item = {v: [v] for v in self.canonical_values_meta}


        # The prefix tree is built from the canonical values
        if args.use_control_symbol:
            self.item_prefix_tree, self.idx2token_ids = self.create_item_prefix_tree()

        if args.is_main_process:
            print(f'compute_{self.mode}_datum_info for Value Linking')
    ...
    def create_item_prefix_tree(self):
        ...
        value_list = list(self.canonical_values_meta.keys())
        # The tokenizer expects a list of strings
        value_ids = self.tokenizer.batch_encode_plus(value_list, add_special_tokens=False).data['input_ids']

        # We don't have a pre-assigned 'emb_idx'
        idx2token_ids = {} 

        if self.args.CBS_type == 1:
            return Trie_link(value_ids, self.tokenizer), idx2token_ids
        elif self.args.CBS_type == 2:
            return Trie_link(value_ids, self.tokenizer), idx2token_ids
        else:
            raise NotImplementedError
    ...
    def get_scope_mask(self, labels):
        ...
        for idx, end_pos in enumerate(end_positions):
            ...
            for step_idx in range(start_idx + 1, end_idx):
                scope_list = self.item_prefix_tree.next_tokens(labels[batch_idx, start_idx:step_idx].tolist())
                if self.args.scope_mask_type == 1:
                    if len(scope_list) < 2: continue
                elif self.args.scope_mask_type == 2:
                    if len(scope_list) < 2 and self.tokenizer.eoi_token_id in scope_list: continue
                elif self.args.scope_mask_type == 3: pass
    ...
    def compute_datum_info(self):
        num_pairs = len(self.training_pairs)
        # Simple 80/10/10 split
        indices = list(range(num_pairs))
        random.Random(self.args.seed).shuffle(indices) # Shuffle with seed for reproducibility
        
        train_end = int(num_pairs * 0.8)
        val_end = int(num_pairs * 0.9)
        ...
        if self.args.is_main_process:
            print(f"{self.mode.capitalize()} split has {len(split_indices)} samples.")

        for task, num in self.task_num.items():
            for _ in range(num):
                self.datum_info += [[task, idx] for idx in split_indices]
    ...
    def getitem_logic(self, idx):
        ...
        training_pair = self.training_pairs[pair_index]
        variation = training_pair['variation']
        canonical_value = training_pair['canonical_value']

        input_field_data = {'variation': variation}
    ...
    def collate_fn(self, batch):
        ...
        batch_entry = {}
        tasks, input_texts, infer_text, output_texts = [], [], [], []
        ...
        batch_entry['input_field_data'] = input_field_data
        batch_entry['output_field_data'] = output_field_data
        batch_entry['input_data'] = {'input_ids': input_ids, 'attention_mask': torch.where(torch.eq(input_ids, self.tokenizer.pad_token_id), 0, 1)}
        batch_entry['complete_text_data'] = {'input_ids': complete_ids, 'attention_mask': torch.where(torch.eq(complete_ids, self.tokenizer.pad_token_id), 0, 1)}
        batch_entry['complete_label_ids'] = labels
        return batch_entry
