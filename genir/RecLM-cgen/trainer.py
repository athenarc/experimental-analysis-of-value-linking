...
class SFTTrainer(nn.Module):
    def __init__(self, args):
        ...
        if 'ValueLinking' in self.args.train_stage:
            self.SFTDatasetClass = ValueLinkingDataset
            self.TrainTaskTemplate = ValueLinking_group
            self.ValTaskTemplate = ValValueLinking_group
            self.TestTaskTemplate = ValValueLinking_group # For now, use the same for testing
            
            training_pairs = []
            # The training pairs file is a jsonl, so read line by line
            with open(os.path.join(args.data_path, 'training_pairs.jsonl'), 'r', encoding='utf-8') as f:
                for line in f:
                    training_pairs.append(json.loads(line))
            
            self.data = {
                'values': load_json(os.path.join(args.data_path, 'values.jsonl')),
                'training_pairs': training_pairs,
                'share_chat_gpt': None, # Not used for this task
            }
        else: # Original logic for recommendation
            self.SFTDatasetClass = SFTDataset
            self.TrainTaskTemplate = Train_task_group_mapping
            self.ValTaskTemplate = Val_task_group_mapping
            self.TestTaskTemplate = Test_task_group_mapping
    ...
    def create_embeddings_(self, batch_size=8, max_length=1024):
        ...
        for i in range(0, len(item_ids), batch_size):
            ...
            model = AutoModelForCausalLM.from_pretrained(self.args.backbone, torch_dtype=torch.bfloat16,
                                                         device_map=self.args.gpu,cache_dir="/data/hdd1/vllm_models/") if model is None else model
            has_updated = True
            print(f'embedding: Batch {i // batch_size + 1}/{(len(item_ids) + batch_size - 1) // batch_size}')
            batch_item_titles = [self.data['metas'][_id][self.args.item_index] for _id in batch_item_ids]
            batch_item_descriptions = [self.data['metas'][_id]['description'] for _id in batch_item_ids]
